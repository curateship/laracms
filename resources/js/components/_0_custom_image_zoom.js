...
function toggleFullWidth(imageZoom, bool) {
    if(window.requestAnimationFrame && !Util.osHasReducedMotion() && imageZoom.animate) { // start expanding animation
...
function initImagesZoom(){
    ...
    if( imageZoom.length > 0 ) {
        ...
        const zoomedImages = localStorage.getItem('images-zoom-light-box')
        if(zoomedImages !== null){
            JSON.parse(zoomedImages).forEach(function(zoomBox){
                // Drop already exit light box;
                const zoomLightBoxBlock = document.getElementById(zoomBox.lightBoxId)
                if(zoomLightBoxBlock !== null){
        ...
        localStorage.setItem('images-zoom-light-box', JSON.stringify(imageZoomArray))
