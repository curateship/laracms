            ...
            <div class="inline-flex items-baseline">
              ...
              <nav class="breadcrumbs text-based padding-left-sm padding-sm" aria-label="Breadcrumbs">
                <ol class="flex flex-wrap gap-xxs">
                  ...
                  <li class="breadcrumbs__item color-contrast-high" aria-current="page">Edit {{ $category->name }}</li>
                </ol>
              </nav>
              <!-- Bread Crumb END -->
        </div>
...
<form action="{{ route('admin.categories.update', $category) }}" method='post'>
  ...
  <fieldset class="margin-bottom-xxs">
    ...
    <div class="margin-bottom-sm">
      <input class="form-control width-100%" type="text" value='{{ old("name", $category->name) }}' name="name" id="inputtitle" placeholder="Enter Your Title" required>
    </div>
    @error('title')
    <p>{{ $message }}</p>
    @enderror
    ...
    <div class="margin-bottom-sm">
      <input class="form-control width-100%" type="text" value='{{ old("slug", $category->slug) }}' name="slug" id="inputtitle" placeholder="Enter Your Slug" required>
    </div>
    @error('slug')
    <p>{{ $message }}</p>
    @enderror
    ...
    <div class="flex justify-end gap-xs">
        ...
        <a class='btn btn--primary' onclick="event.preventDefault();document.getElementById('delete_category_{{ $category->id }}').submit()"href="#">Delete Category</a>
    </div>
   </form>
