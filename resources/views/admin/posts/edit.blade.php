    ...
    <!-- Select Category Dropdown Autocomplete END -->
      ...
      @foreach(\App\Models\TagsCategories::all() as $key=> $tag_category)
          <div class="grid margin-bottom-sm">
              ...
              <select name="tag_category_{{ $tag_category->id }}[]" id="tag_category_{{ $tag_category->id }}" class="site-tag-pills" data-id="{{ $tag_category->id }}" data-placeholder="Select a Tags for {{ $tag_category->name }}" multiple>
