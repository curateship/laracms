            ...
            <div class="inline-flex items-baseline">
              ...
              <nav class="breadcrumbs text-based padding-left-sm padding-sm" aria-label="Breadcrumbs">
                <ol class="flex flex-wrap gap-xxs">
                  ...
                  <li class="breadcrumbs__item color-contrast-high" aria-current="page">Edit {{ $tag->name }}</li>
                </ol>
              </nav>
              <!-- Bread Crumb END -->
        </div>
...
<form action="{{ route('admin.tags.update', $tag) }}" method='post'>
  ...
  <fieldset class="margin-bottom-xxs">
    ...
    <div class="margin-bottom-sm">
      <input class="form-control width-100%" type="text" value='{{ old("name", $tag->name) }}' name="name" id="inputtitle" placeholder="Enter Your Title" required>
    </div>
    @error('title')
    <p>{{ $message }}</p>
    @enderror
    ...
    <div class="margin-bottom-sm">
      <input class="form-control width-100%" type="text" value='{{ old("slug", $tag->slug) }}' name="slug" id="inputtitle" placeholder="Enter Your Slug" required>
    </div>
    @error('slug')
    <p>{{ $message }}</p>
    @enderror
    ...
    <div class="flex justify-end gap-xs">
        <button class="btn btn--subtle">Update</button>
        <a class='btn btn--primary' onclick="event.preventDefault();document.getElementById('delete_tag_{{ $tag->id }}').submit()"href="#">Delete Category</a>
    </div>
   </form>

   <form id='delete_tag_{{ $tag->id }}' method='post' action="{{ route('admin.tags.destroy', $tag) }}">
        @csrf
        @method('DELETE')
    </form>
</div>
