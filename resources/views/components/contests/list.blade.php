...
@section('content')
    <div class="padding-md">
        <div class="margin-bottom-lg">
            @foreach($contests_list as $status => $contests)
                @if(count($contests) > 0)
                    <h3 class="margin-bottom-sm contest-status contest-status-{{$status}}">{{$status}}</h3>
                    <ul class="grid-auto-xs grid-auto-sm@sm grid-auto-lg@md gap-md margin-bottom-md">
