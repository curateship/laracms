@if(count(range(1, $items->lastPage())) > 1)
    <nav class="pagination text-sm" aria-label="Pagination">
        <ol class="pagination__list flex flex-wrap gap-xs justify-center padding-sm">
            ...
            <li>
                <a href="{{ $items->appends(request()->query())->url($items->currentPage()-1) }}" class="pagination__item {{ ($items->currentPage() == 1) ? ' pagination__item--disabled' : '' }}" aria-label="Go to previous page">
            ...
            @if($items->currentPage() > 3)
                <li class="display@sm">
                    <a href="{{ $items->appends(request()->query())->url(1) }}" class="pagination__item {{ ($items->currentPage() == 1) ? ' pagination__item--selected' : '' }}" aria-label="Go to page 1">1</a>
                </li>
            @endif
            ...
            @foreach(range(1, $items->lastPage()) as $i)
                @if($i >= $items->currentPage() - 2 && $i <= $items->currentPage() + 2)
                    <li class="display@sm">
                        <a href="{{ $items->appends(request()->query())->url($i) }}" class="pagination__item {{ ($items->currentPage() == $i) ? ' pagination__item--selected' : '' }}" aria-label="Go to page {{ $i }}">{{ $i }}</a>
                    </li>
                @endif
            @endforeach
            ...
            @if($items->currentPage() < $items->lastPage() - 2)
                <li class="display@sm">
                    <a href="{{ $items->appends(request()->query())->url($items->lastPage()) }}" class="pagination__item {{ ($items->currentPage() == $items->lastPage()) ? ' pagination__item--selected' : '' }}" aria-label="Go to page {{ $items->lastPage() }}">{{ $items->lastPage() }}</a>
                </li>
            @endif

            <li>
                <a href="{{ $items->appends(request()->query())->url($items->currentPage()+1) }}" class="pagination__item {{ ($items->currentPage() == $items->lastPage()) ? ' pagination__item--disabled' : '' }}" aria-label="Go to next page">
                    <span>Next</span>
