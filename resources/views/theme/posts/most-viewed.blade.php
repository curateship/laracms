...
<div class="inline-flex items-baseline">
  ...
  <div class="select inline-block js-select" data-trigger-class="reset text-sm color-contrast-high text-underline inline-flex items-center cursor-pointer js-tab-focus">
    <select name="selectThis" id="selectThis" class="most-posts-filter">
        <option value="/most-viewed" {{request()->get('filter') == null ? 'selected' : ''}}>All Time</option>
        <option value="/most-viewed?filter=today" {{request()->get('filter') == 'today' ? 'selected' : ''}}>Today</option>
        <option value="/most-viewed?filter=week" {{request()->get('filter') == 'week' ? 'selected' : ''}}>Week</option>
        <option value="/most-viewed?filter=month" {{request()->get('filter') == 'month' ? 'selected' : ''}}>Month</option>
    </select>
